...
| FR2-5 | **生图 Skill 节点**：普通画布（canvas.js）新节点类型（2026-09-19 范围修订：用户所指"API/ModelScope 生图节点"即普通画布 generator/msgen 节点，smart-canvas 为 composer 流无生成节点，其接入留待后续迭代）；节点上 skill 下拉（按来源分组）；显示名称/版本/来源徽标；模式切换（快速/智能） |
