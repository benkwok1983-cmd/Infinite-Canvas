...
本节复核 Zcode 针对第一次校审提交的修复，重点检查敏感日志、API 回退持久化、variant 缓存、空提示词生图、预览参数、Skill 覆盖安装事务及新增测试。校审仍以只读代码审查为主，未修改项目代码。

### 已确认完成的修复

1. 删除了输出完整 argv 的调试日志，降低 API Key 进入日志的风险。
2. 前端 Provider 保存请求补充了 `allow_api_fallback` 字段。
3. 缓存键纳入了原始 variant、解析后的 variant 标识及模板指令，降低不同样板串缓存的风险。
4. API 生图允许空 prompt，并改为业务层校验：提示词、Skill、参考图至少存在一项；三者全空仍拒绝。
5. `SKILL.md` 增加 1MB 大小上限。
6. GitHub 和 zip 覆盖安装接入了备份、移动失败恢复逻辑。
7. 前端预览请求已发送 variant 和 model。
8. 缓存写入采用临时文件加 `os.replace`，改善单次写入中断时的文件完整性。

### 仍需修复的问题

#### P1：回归测试可能修改真实 Provider 配置

`tests/test_skill_library.py` 中的 `test_allow_api_fallback_put_get_roundtrip` 通过真实 `/api/providers` 读写配置，并将值强制设为 `False`，不一定恢复测试前状态。若测试中途异常，恢复也可能不执行。

建议将 `API_PROVIDERS_FILE` monkeypatch 到临时目录；若必须使用真实路径，则保存原文件字节并在 `finally` 中原样恢复、校验恢复结果，不能假定原始值为 `False`。

#### P2：缓存原子写仍有跨 key 并发竞争

临时文件名固定为缓存文件名加 `.tmp`，但锁按缓存 key 分离。不同 key 可以同时写同一个临时文件，导致记录丢失、替换失败或异常被吞掉。

建议增加缓存文件级全局锁，在锁内完成读取、合并、写唯一临时文件、flush、fsync 和 `os.replace`。临时文件应使用同目录唯一名称。

#### P2：预览请求中的 model/provider 在后端可能被丢弃

前端已经发送 variant/model，但 `SkillPreviewRequest` 和 `SkillSelection` 后端链路没有完整接收、保存和使用 model/provider；旧的快速预览接口也没有统一解析 variant。因此预览仍可能与实际生成使用不同模型或样板。

建议补齐请求模型字段，统一快速预览和智能预览的 variant 解析，并增加预览与实际生成参数一致性测试。

#### P2：覆盖安装事务没有覆盖元数据写入

目录移动成功后，旧目录备份可能已经被删除，而调用方随后才写 `.skill_meta.json`。如果元数据写入失败，旧 Skill 无法恢复，安装结果可能不完整。

建议将目录替换和元数据写入纳入同一事务；只有元数据写入并验证成功后才删除备份，任一步失败都恢复旧目录。GitHub 和 zip 两条路径都应增加元数据写入失败测试。

### 测试与验证说明

Zcode 汇报 62 项测试全绿，但静态测试方法计数为 21 + 11 + 28 = 60。可能是参数化用例或收集口径不同，建议在总结中附实际测试命令和测试框架原始摘要。

当前提交未见 GitHub CI 状态。本次二次校审也未能在当前 Codex 环境中独立运行测试或验证服务重启，因此测试全绿和服务已重启属于 Zcode 执行结果陈述，不是本次独立复现结论。

### 建议处理顺序

1. 隔离真实 Provider 配置测试。
2. 修复缓存文件级并发写入。
3. 打通预览的 model/provider/variant 全链路。
4. 将元数据写入纳入 Skill 安装事务。
5. 重跑完整测试并保存原始摘要。
6. 在独立分支完成手工验收后，再评估是否合并到稳定分支。

### 二次校审结论

Zcode 的修复方向正确，主要安全和业务问题已有明显改善，但仍未完全闭环。建议当前结论为：

> **有条件通过二次校审，继续保留在 `zcode/iteration-01` 独立分支；完成上述 P1/P2 修复并通过独立回归验证后，再进入合并评估。**
